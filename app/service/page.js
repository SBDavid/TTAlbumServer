module.exports = app => {
    class page extends app.Service {
        * get(url) {
            // 更具pageId查找page
            var urlmaps = yield this.ctx.model.Urlmaps.find({url: url});
            // 筛选日期
            ...
            var pages = yield this.ctx.model.Pages.find({id: urlmaps[0].pageId});
            // 判断页面pages时候有效
            if (pages.length === 0) {
                return{
                    success: false,
            ...
            var repositories = pages[0].repository.sort((r1, r2) => {
